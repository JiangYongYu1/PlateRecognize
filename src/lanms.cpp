...
namespace lanms {
    ...
    PolyMerger::PolyMerger(): score(0), poly_sin(0), poly_cos(0), nr_polys(0) {
				memset(data, 0, sizeof(data));
			}
    void PolyMerger::add(const Polygon &p_given) {
        Polygon p;
        if (nr_polys > 0) {
    ...
    Polygon PolyMerger::normalize_poly(
            const Polygon &ref,
            const Polygon &p) {
        ...
        r.score = p.score;
        r.poly_sin = p.poly_sin;
        r.poly_cos = p.poly_cos;
        return r;
    }
	...
	std::vector<std::vector<float>> polys2floats(const std::vector<lanms::Polygon> &polys) {
		...
		for (size_t i = 0; i < polys.size(); i ++) {
			...
			ret.emplace_back(std::vector<float>{
					...
					float(p.score), float(p.poly_sin),
					float(p.poly_cos),
					});
		}
